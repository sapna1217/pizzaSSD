...
export const placeOrder = (token, subtotal,coordinates) => async (dispatch, getState) => {
    ...
    try {
        ...
        await axios.post('http://localhost:8070/api/orders/placeorder', { token, subtotal, currentUser, cartItems,coordinates })
...
export const getUserOrders = () => async (dispatch , getState) => {
    ...
    try {
        ...
        const response = await axios.post('http://localhost:8070/api/orders/getuserorders' , {userid : currentUser._id})
...
export const deleteOrderAction = (OrderID) => async dispatch => {
    ...
    try {
        const response = await axios.delete(`http://localhost:8070/api/orders/delete/Order/${OrderID}`)
...
export const updateOrderDeliveryAction = (updateisDeliverd, orderId) => async dispatch => {
    ...
    dispatch({ type: 'UPDATE_ORDER_DELIVERY_VERIFICATION_REQUEST' })
        ...
        try {
            const response = await axios.put(`http://localhost:8070/api/orders/update/order/status/${orderId}`, updateisDeliverd)
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
...
export const updaterefundrequestAction = (updatesendrefundStatus, orderId) => async dispatch => {
    ...
    dispatch({ type: 'UPDATE_REFUND_REQUEST' })
        ...
        try {
            const response = await axios.put(`http://localhost:8070/api/orders/update/order/refund/request/${orderId}`, updatesendrefundStatus)
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
...
export const updateuserrefundrequestAction = (updateorderStatus, orderId) => async dispatch => {
    ...
    dispatch({ type: 'UPDATE_REFUND_REQUEST' })
        ...
        try {
            const response = await axios.put(`http://localhost:8070order/refund/request/user/${orderId}`, updateorderStatus)
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
