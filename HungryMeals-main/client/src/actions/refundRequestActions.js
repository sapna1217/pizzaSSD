...
export const updateStatusAction = (updatestatus, salesid, val) => async dispatch => {
    ...
    if (val === true) {
        ...
        try {
            const response = await axios.put(`http://localhost:8070/api/orders/update/transactionstatus/${salesid}`, updatestatus)
        ...
        try {
            const response = await axios.put(`http://localhost:8070{salesid}`, updatestatus)
            const Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
