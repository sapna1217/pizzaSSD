...
export default function Newsfeedmanagement() {
    ...
    useEffect(() => {
        function getNews() {
            axios.get("http://localhost:8070/api/newsfeed/getallnews").then((res) => {
    ...
    function getCurrentNews(newsId) {
        ...
        axios.get(`http://localhost:8070/api/newsfeed/getcurrentnews/${newsId}`).then((res) => {
    ...
    useEffect(() => {
        const result = news.filter(news => {
            return news.header.toLowerCase().includes(search.toLowerCase());
        });
